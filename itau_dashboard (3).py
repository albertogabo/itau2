...
if st.session_state.page == "home":
    ...
    # Country toggle — clickable buttons
    st.markdown("**Seleccionar base de consumo**")
    ct1, ct2, _ = st.columns([1, 1, 4])
    with ct1:
        col_active = "colombia" in st.session_state.country.lower() or "Colombia" in st.session_state.country
        if st.button("🇨🇴 Colombia", key="tog_col",
                     type="primary" if "Colombia" in st.session_state.country else "secondary",
                     use_container_width=True):
            st.session_state.country = "🇨🇴 Colombia"; st.rerun()
    with ct2:
        if st.button("🇵🇦 Panamá", key="tog_pan",
                     type="primary" if "Panamá" in st.session_state.country else "secondary",
                     use_container_width=True):
            st.session_state.country = "🇵🇦 Panamá"; st.rerun()
    ...
    with col1:
        clicked_busqueda = st.button(
            f"""🔍  Búsqueda por ID
{flag} Consumo Bcos {label} · {len(df_active)} registros""",
            key="go_busqueda", use_container_width=True
        )
        st.markdown(f"""
<div style="background:#fff;border:2px solid #FF6200;border-radius:16px;padding:24px;text-align:center;box-shadow:0 2px 12px rgba(255,98,0,0.1);margin-top:-8px">
  <div style="font-size:44px;margin-bottom:10px">🔍</div>
  <div style="font-size:19px;font-weight:800;color:#222;margin-bottom:6px">Búsqueda por ID</div>
  <div style="font-size:13px;color:#888;line-height:1.6">Consulta cupos disponibles y riesgo país por ID Solcred</div>
  <div style="margin-top:10px;display:inline-block;background:#fff3ec;border-radius:20px;padding:4px 14px;font-size:13px;font-weight:700;color:#FF6200">{flag} Consumo Bcos {label}</div>
  <div style="margin-top:6px;font-size:13px;font-weight:700;color:#FF6200">{len(df_active)} registros disponibles →</div>
</div>
""", unsafe_allow_html=True)
        if clicked_busqueda:
...
""", unsafe_allow_html=True)
        st.markdown("")
        clicked_alertas = st.button(
            f"⚠  Alertas de Riesgo País  🔴 {len(criticos)} críticas  🟡 {len(normales)} alertas",
            key="go_alertas", use_container_width=True
        )
        if clicked_alertas:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Country switcher — clickable buttons
    st.markdown("**Base de consumo:**")
    sc1, sc2_, _ = st.columns([1,1,4])
    with sc1:
        if st.button("🇨🇴 Colombia", key="bus_col",
                     type="primary" if "Colombia" in st.session_state.country else "secondary",
                     use_container_width=True):
            st.session_state.country = "🇨🇴 Colombia"; setSelectedId_reset = True; st.rerun()
    with sc2_:
        if st.button("🇵🇦 Panamá", key="bus_pan",
                     type="primary" if "Panamá" in st.session_state.country else "secondary",
                     use_container_width=True):
            st.session_state.country = "🇵🇦 Panamá"; st.rerun()
...
""", unsafe_allow_html=True)
        ...
        for i, (tipo, (col_c, linea_r)) in enumerate(prod_map.items()):
            ...
            # % = consumo riesgo país / límite riesgo país
            con_r   = float(rp_l["ConsumoTotal"].values[0]) if len(rp_l) else None
            pct     = min(100, (con_r / lim) * 100) if (lim and lim > 0 and con_r is not None) else 0
  ...
  <div style="font-size:10px;color:#aaa;margin-top:4px">Límite: {fmt(lim)} · Consumo: {fmt(con_r)} · <span style="color:{bc_col};font-weight:700">{pct:.1f}% del límite consumido</span></div>"""
